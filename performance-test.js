...
class PerformanceTest {
  ...
  async findServerPort() {
    ...
    for (let port = 55555; port < 55565; port++) {
      try {
        const response = await axios.get(`http://127.0.0.1:${port}/health`, { timeout: 1000 });
  ...
  async waitForServer(maxAttempts = 60) {
    ...
    for (let i = 0; i < maxAttempts; i++) {
      try {
        const response = await axios.get(`${this.baseUrl}:${this.port}/health`, { timeout: 2000 });
        if (response.data.status === 'healthy' || response.data.status === 'ok') {
  ...
  async testServerStartup() {
    ...
    console.log('❌ No running server found. This test requires a running server.');
    this.results.serverStartup = {
  ...
  async testFirstAnalysis() {
    ...
    try {
      const response = await axios.post(`${this.baseUrl}:${this.port}/analyze`, {
      ...
      console.log(`✅ First analysis completed in ${responseTime.toFixed(2)}ms`);
      console.log(`   - Server processing: ${response.data.processing_time_ms}ms`);
  ...
  async testSubsequentAnalyses() {
    ...
    for (let i = 0; i < this.testTexts.length; i++) {
      ...
      try {
        ...
        console.log(`${i + 1}/10 - ${responseTime.toFixed(2)}ms - ${text.substring(0, 30)}...`);
    ...
    console.log(`📊 Average response time: ${this.results.subsequentAnalyses.averageResponseTime.toFixed(2)}ms`);
    console.log(`📊 Average processing time: ${this.results.subsequentAnalyses.averageProcessingTime.toFixed(2)}ms`);
  }
  ...
  async testPerformanceEndpoint() {
    ...
    try {
      const response = await axios.get(`${this.baseUrl}:${this.port}/performance`, { timeout: 5000 });
      
      this.results.performanceStats = response.data;
      
      console.log('✅ Performance stats retrieved:');
      console.log(`   - Memory usage: ${response.data.ai_manager?.memory_usage_mb}MB`);
      console.log(`   - Models loaded: ${response.data.ai_manager?.models_loaded}`);
      console.log(`   - Cache size: ${response.data.ai_manager?.cache_size}`);
      console.log(`   - Background loading: ${response.data.ai_manager?.background_loading}`);
      
      if (response.data.system) {
        console.log(`   - CPU usage: ${response.data.system.cpu_percent}%`);
        console.log(`   - Memory percent: ${response.data.system.memory_percent}%`);
      }
  ...
  async testConcurrentRequests() {
    ...
    for (let i = 0; i < concurrentRequests; i++) {
      ...
      const requestPromise = axios.post(`${this.baseUrl}:${this.port}/analyze`, {
        text,
        include_toxicity: true,
        include_sentiment: true
      }, { timeout: 10000 });
    ...
    try {
      ...
      console.log(`   - Throughput: ${this.results.concurrentRequests.throughput} req/sec`);
  ...
  generateSummary() {
    ...
    if (this.results.firstAnalysis?.success) {
      summary['First Response Time'] = `${this.results.firstAnalysis.responseTime.toFixed(2)}ms`;
    }
    
    if (this.results.subsequentAnalyses) {
      summary['Avg Subsequent Response'] = `${this.results.subsequentAnalyses.averageResponseTime.toFixed(2)}ms`;
    }
    
    if (this.results.performanceStats?.ai_manager) {
      summary['Memory Usage'] = `${this.results.performanceStats.ai_manager.memory_usage_mb}MB`;
      summary['Models Loaded'] = this.results.performanceStats.ai_manager.models_loaded;
      summary['Cache Size'] = this.results.performanceStats.ai_manager.cache_size;
    }
    
    if (this.results.concurrentRequests?.throughput) {
      summary['Throughput'] = `${this.results.concurrentRequests.throughput} req/sec`;
    }
...
async function runPerformanceTest() {
  ...
  try {
    ...
    console.log('\n🎯 Performance Targets:');
    const firstResponse = tester.results.firstAnalysis?.responseTime || Infinity;
    const avgSubsequent = tester.results.subsequentAnalyses?.averageResponseTime || Infinity;
    
    console.log(`   First Response: ${firstResponse < 5000 ? '✅' : '❌'} ${firstResponse.toFixed(2)}ms (target: <5000ms)`);
    console.log(`   Avg Subsequent: ${avgSubsequent < 500 ? '✅' : '❌'} ${avgSubsequent.toFixed(2)}ms (target: <500ms)`);
    
    const memoryMB = tester.results.performanceStats?.ai_manager?.memory_usage_mb || 0;
    console.log(`   Memory Usage: ${memoryMB < 1000 ? '✅' : '❌'} ${memoryMB}MB (target: <1000MB)`);
